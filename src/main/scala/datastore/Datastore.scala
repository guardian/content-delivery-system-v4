...
trait Datastore {
  ...
  def close():Unit
  def setMulti(section:String, params:Map[String,String]):Future[List[Boolean]]
  def getMulti(section:String, keys:List[String]):Future[Map[String,String]]

  def set(section:String, key:String, value:String):Future[Boolean] =
    setMulti(section,Map(key->value)).flatMap((x:List[Boolean])=>Future(x.head))

  def setSync(section:String, key: String, value: String, timeout: Duration):Boolean =
    Await.result(set(section,key,value), timeout).asInstanceOf[Boolean]
  ...
  def substituteString(input:String):Future[String] = {
    def nextSubstitution(string:String,futures:List[Future[String]]):List[Future[String]] = {
      ...
      val newfutures = futures :+ to_sub.map(x=>string.substring(0,end).replaceAll(identifier,x))
